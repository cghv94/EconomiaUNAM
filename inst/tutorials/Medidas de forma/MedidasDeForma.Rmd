...
<p style="text-align: justify">**DISTRIBUCIÓN SIMÉTRICA**
<p style="text-align: justify">La *distribución simétrica* es cuando los valores del lado izquierda de la media se encuentran en igualdad numérica que los valores de la derecha, también cuando existe el mismo número de desviaciones positivas y negativas. 
<center>
...
```
<p style="text-align: justify">*Media y mediana*
<p style="text-align: justify">Obtendremos la media y mediana. 
```{r}
data.frame(Mediana = median(simetrica), Media = mean(simetrica))
```
<p style="text-align: justify">*Histograma con línea de densidad*
...
<p style="text-align: justify">***Asimetría positiva:*** La asimetría positiva se da cuando la media obtenida es mayor que la moda. Cuando esto se da, la gráfica se inclina de lado izquierdo. 
<center>
...
```
<p style="text-align: justify">*Histograma*
...
<p style="text-align: justify">***Asimetría negativa:*** La asimetría negativa implica que la media es menor que la moda obtenida. La gráfica se inclina hacia derecha. 
...
<center>
 ...
 <p style="text-align: justify">***Distribución mesocúrtica:*** La distribución mesocúrtica se obtiene cuando la curtosis es igual a cero, o bien, se tiene una distribución normal. 
...
```
 ...
 <p style="text-align: justify">***Distribución leptocúrtica:*** La distribución leptocúrtica se obtiene cuando la curtosis resulta ser mayor a cero, gráficamente los datos se concentran más en la media, por lo que la curva se ve más alargada o puntiaguda. 
...
``` 
 ...
 <p style="text-align: justify">***Distribución platicúrtica:*** La distribución platicúrtica es el resultado de una concentración muy baja de los datos en la media, es decir, que la curtosis resulta ser menor a cero. Gráficamente la forma de la curva es más aplastada, pues no existe una concentración de los datos. 
...
<p style="text-align: justify">Para este ejercicio vamos a importar los datos de la deuda de CDMX, y después haremos el resumen estadístico del mismo. 

```{r}
deuda <- read.csv("https://cghv94.github.io/deuda-cdmx.csv")
summary(deuda)
``` 
<p style="text-align: justify"> Ahora podemos crear el histograma de la disposición inicial del crédito:
```{r}
hist(deuda$disposicion_inicial_credito) 
``` 
<p style="text-align: justify"> Ahora podemos obtener la media, moda y mediana. 
```{r}
...
``` 
<p style="text-align: justify"> Ahora creamos el *Histograma con línea de densidad*
```{r}
...
``` 
<p style="text-align: justify">Y terminamos obteniendo el coeficiente de asimetría y la curtosis. 
```{r}
